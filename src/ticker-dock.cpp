...
// ─────────────────────────────────────────────────────────────────────────────
static const char *S_ROOT = "background-color:#0f172a; color:#e2e8f0;";

static const char *S_INPUT = "QLineEdit { background:#1e293b; border:1px solid #475569;"
			     "  border-radius:4px; color:#e2e8f0; padding:4px 8px; }"
			     "QLineEdit:focus { border-color:#3b82f6; }";

static const char *S_COMBO = "QComboBox { background:#1e293b; border:1px solid #475569;"
			     "  border-radius:4px; color:#e2e8f0; padding:4px 8px; }"
			     "QComboBox QAbstractItemView { background:#1e293b; color:#e2e8f0; }";

static const char *S_BTN_PRIMARY = "QPushButton { background:#2563eb; color:#fff; border-radius:4px;"
				   "  padding:5px 12px; font-weight:600; border:none; }"
				   "QPushButton:hover { background:#3b82f6; }"
				   "QPushButton:disabled { background:#334155; color:#64748b; }";

static const char *S_BTN_GREEN = "QPushButton { background:#166534; color:#86efac; border-radius:4px;"
				 "  padding:5px 12px; font-weight:600; border:none; }"
				 "QPushButton:hover { background:#15803d; }";

static const char *S_BTN_DANGER = "QPushButton { background:#7f1d1d; color:#fca5a5; border-radius:4px;"
				  "  padding:4px 8px; border:none; }"
				  "QPushButton:hover { background:#991b1b; }";

static const char *S_BTN_NEUTRAL = "QPushButton { background:#334155; color:#cbd5e1; border-radius:4px;"
				   "  padding:4px 10px; border:none; }"
				   "QPushButton:hover { background:#475569; }";

static const char *S_SECTION_HDR = "QPushButton { background:#1e293b; color:#e2e8f0; font-weight:600;"
				   "  font-size:12px; text-align:left; padding:8px 10px;"
				   "  border:1px solid #334155; border-radius:4px; }"
				   "QPushButton:hover { background:#263347; }";

static const char *S_CARD = "background:#1e293b; border:1px solid #334155; border-radius:6px;";

static const char *S_LABEL_MUTED = "color:#94a3b8; font-size:11px;";
	...
	obs_enum_sources(
		[](void *param, obs_source_t *src) -> bool {
			if (strcmp(obs_source_get_id(src), "obs_ticker_cpp") == 0) {
				...
				if (n)
					strncpy(static_cast<char *>(param), n, 511);
				return false;
			}
			return true;
		},
...
void ticker_push_settings(const TickerSettings &s, const QString &messagesStr)
{
	...
	auto toAbgr = [](const QString &hex) -> uint32_t {
		QColor c(hex);
		return (0xFF000000u) | ((uint32_t)c.blue() << 16) | ((uint32_t)c.green() << 8) | ((uint32_t)c.red());
	};
...
CollapsibleSection::CollapsibleSection(const QString &title, bool startOpen, QWidget *parent)
	: QWidget(parent),
	  m_open(startOpen)
{
	...
	m_header = new QPushButton(QString("%1  %2").arg(m_open ? "▾" : "▸").arg(title));
	...
	m_body = new QWidget;
	m_body->setStyleSheet("background:#0a0f1a; border:1px solid #334155;"
...
// ─────────────────────────────────────────────────────────────────────────────
TickerDock::TickerDock(QWidget *parent) : QWidget(parent)
{
	s_instance = this;
	...
	m_messages = {
		{QUuid::createUuid().toString(QUuid::WithoutBraces), "Welcome to the stream!", true, false},
		{QUuid::createUuid().toString(QUuid::WithoutBraces), "Don't forget to follow!", true, false},
		{QUuid::createUuid().toString(QUuid::WithoutBraces), "New video coming soon!", false, false},
	};
	...
	m_tabs->setStyleSheet("QTabWidget::pane { border:0; background:#0f172a; }"
	...
	connect(this, &TickerDock::messagesChanged, m_goLiveTab, &GoLiveTab::refresh, Qt::QueuedConnection);
	connect(this, &TickerDock::messagesChanged, m_libraryTab, &LibraryTab::refresh, Qt::QueuedConnection);
	connect(this, &TickerDock::settingsChanged, m_goLiveTab, &GoLiveTab::refresh, Qt::QueuedConnection);
...
void TickerDock::removeMessage(const QString &id)
{
	...
	bool anyActive =
		std::any_of(m_messages.begin(), m_messages.end(), [](const TickerMessage &m) { return m.active; });
	if (!anyActive)
...
static TickerSettings jsonToSettings(const QJsonObject &o, const TickerSettings &def)
{
	...
	if (o.contains("speed"))
		s.speed = o["speed"].toInt();
	if (o.contains("backgroundColor"))
		s.backgroundColor = o["backgroundColor"].toString();
	if (o.contains("fontFamily"))
		s.fontFamily = o["fontFamily"].toString();
	if (o.contains("fontStyleName"))
		s.fontStyleName = o["fontStyleName"].toString();
	if (o.contains("fontSize"))
		s.fontSize = o["fontSize"].toInt();
	if (o.contains("color"))
		s.color = o["color"].toString();
	if (o.contains("itemSeparator"))
		s.itemSeparator = o["itemSeparator"].toString();
	if (o.contains("fontScaleX"))
		s.fontScaleX = o["fontScaleX"].toDouble();
	if (o.contains("fontScaleY"))
		s.fontScaleY = o["fontScaleY"].toDouble();
	if (o.contains("showClock"))
		s.showClock = o["showClock"].toBool();
	if (o.contains("clockFormat"))
		s.clockFormat = o["clockFormat"].toString();
	if (o.contains("clockFontSize"))
		s.clockFontSize = o["clockFontSize"].toInt();
	if (o.contains("clockScaleX"))
		s.clockScaleX = o["clockScaleX"].toDouble();
	if (o.contains("clockScaleY"))
		s.clockScaleY = o["clockScaleY"].toDouble();
	if (o.contains("clockZoneWidth"))
		s.clockZoneWidth = o["clockZoneWidth"].toInt();
	if (o.contains("clockSepColor"))
		s.clockSepColor = o["clockSepColor"].toString();
	if (o.contains("isLive"))
		s.isLive = o["isLive"].toBool();
	return s;
}
...
GoLiveTab::GoLiveTab(TickerDock *dock, QWidget *parent) : QWidget(parent), m_dock(dock)
{
	setStyleSheet(S_ROOT);
	auto *outer = new QVBoxLayout(this);
	...
	connect(m_quickInput, &QLineEdit::returnPressed, this, &GoLiveTab::onQuickAdd);
...
void GoLiveTab::rebuildQueue()
{
	...
	for (const auto &msg : msgs) {
		...
		row->setStyleSheet("background:#1e293b; border:1px solid #334155;"
				   "border-radius:5px;");
		...
		editInput->hide();
		rowLayout->insertWidget(rowLayout->indexOf(txt), editInput, 1);

		// Action buttons
		auto *saveBtn = new QPushButton("✓");
		auto *cancelBtn = new QPushButton("✕");
		auto *editBtn = new QPushButton("Edit");
		auto *saveLibBtn = (msg.temporary) ? new QPushButton("Save") : nullptr;
		auto *removeBtn = new QPushButton("✕");

		saveBtn->setStyleSheet(S_BTN_GREEN);
		cancelBtn->setStyleSheet(S_BTN_NEUTRAL);
		editBtn->setStyleSheet(
			"QPushButton { background:#1e3a5f; color:#93c5fd; border-radius:4px; padding:3px 8px; font-size:10px; border:none; } QPushButton:hover { background:#1d4ed8; }");
		if (saveLibBtn)
			saveLibBtn->setStyleSheet(S_BTN_NEUTRAL);
		removeBtn->setStyleSheet(S_BTN_DANGER);

		for (auto *b : {saveBtn, cancelBtn, editBtn, removeBtn}) {
			if (b) {
				b->setFixedWidth(b == editBtn ? 40 : 28);
				b->setCursor(Qt::PointingHandCursor);
			}
		}
		if (saveLibBtn) {
			saveLibBtn->setFixedWidth(48);
			saveLibBtn->setCursor(Qt::PointingHandCursor);
		}
		...
		if (saveLibBtn)
			rowLayout->addWidget(saveLibBtn);
		rowLayout->addWidget(removeBtn);

		QString capturedId = msg.id;

		auto finishEdit = [this, capturedId, editInput, editBtn, saveBtn, cancelBtn, saveLibBtn,
				   removeBtn](bool save) {
			if (save) {
				QString t = editInput->text().trimmed();
				if (!t.isEmpty()) {
					m_dock->updateMessage(capturedId, t);
					// UI refresh will be triggered by messagesChanged signal (queued)
				}
			} else {
				// Restore state if canceled
				editInput->hide();
				saveBtn->hide();
				cancelBtn->hide();
				editBtn->show();
		...
		connect(editBtn, &QPushButton::clicked,
			[txt, editInput, editBtn, saveBtn, cancelBtn, saveLibBtn, removeBtn]() {
				txt->hide();
				editInput->show();
				editInput->setFocus();
				editInput->selectAll();
		...
		connect(editInput, &QLineEdit::returnPressed, [finishEdit]() { finishEdit(true); });
		connect(saveBtn, &QPushButton::clicked, [finishEdit]() { finishEdit(true); });
		connect(cancelBtn, &QPushButton::clicked, [finishEdit]() { finishEdit(false); });

		if (saveLibBtn) {
			connect(saveLibBtn, &QPushButton::clicked,
				[this, capturedId]() { m_dock->saveToLibrary(capturedId); });
		}

		connect(removeBtn, &QPushButton::clicked, [this, capturedId]() { m_dock->toggleMessage(capturedId); });
...
LibraryTab::LibraryTab(TickerDock *dock, QWidget *parent) : QWidget(parent), m_dock(dock)
{
	setStyleSheet(S_ROOT);
	auto *outer = new QVBoxLayout(this);
	outer->setContentsMargins(8, 8, 8, 8);
...
void LibraryTab::rebuildList()
{
	...
	for (const auto &msg : msgs) {
		...
		auto *row = new QWidget;
		row->setStyleSheet(
			locked ? "background:#111827; border:1px solid #1e3a5f;" // dimmed blue tint = in queue
				 "border-radius:6px;"
		...
		if (locked) {
			...
			addQueueBtn->setStyleSheet("QPushButton { background:#14532d; color:#4ade80; border:none;"
						   "  border-radius:4px; padding:3px 10px; font-size:11px; }"
						   "QPushButton:hover { background:#166534; }");
			editBtn->setStyleSheet("QPushButton { background:#1e3a5f; color:#93c5fd; border:none;"
					       "  border-radius:4px; padding:3px 10px; font-size:11px; }"
					       "QPushButton:hover { background:#1d4ed8; }");
			saveBtn->setStyleSheet(S_BTN_GREEN);
			cancelBtn->setStyleSheet(S_BTN_NEUTRAL);
			deleteBtn->setStyleSheet("QPushButton { background:#3b0d0d; color:#fca5a5; border:none;"
						 "  border-radius:4px; padding:3px 10px; font-size:11px; }"
			...
			connect(editBtn, &QPushButton::clicked, this,
				[displayLabel, editInput, editBtn, addQueueBtn, saveBtn, cancelBtn, deleteBtn]() {
					displayLabel->hide();
					editInput->show();
					editInput->setFocus();
					editInput->selectAll();
			...
			auto finishEdit = [this, capturedId, editInput, editBtn, addQueueBtn, saveBtn, cancelBtn,
					   deleteBtn](bool save) {
				if (save) {
					QString t = editInput->text().trimmed();
					if (!t.isEmpty()) {
			...
			connect(editInput, &QLineEdit::returnPressed, this, [finishEdit]() { finishEdit(true); });
			connect(saveBtn, &QPushButton::clicked, this, [finishEdit]() { finishEdit(true); });
			connect(cancelBtn, &QPushButton::clicked, this, [finishEdit]() { finishEdit(false); });
...
FormattingTab::FormattingTab(TickerDock *dock, QWidget *parent) : QWidget(parent), m_dock(dock)
{
	setStyleSheet(S_ROOT);
	...
	auto *layoutSec = new CollapsibleSection("Layout & Colors", true);
	{
		...
		// Height row
		{
			...
			auto *lbl = new QLabel("Height");
			lbl->setStyleSheet(S_LABEL_MUTED);
			lbl->setSizePolicy(QSizePolicy::Preferred, QSizePolicy::Fixed);
			m_heightSlider = new QSlider(Qt::Horizontal);
			m_heightSlider->setRange(30, 200);
			m_heightSlider->setValue(m_dock->settings().height);
			...
			m_heightVal->setStyleSheet("color:#e2e8f0;font-size:11px;");
			m_heightVal->setMinimumWidth(36);
			m_heightVal->setAlignment(Qt::AlignRight | Qt::AlignVCenter);
			rowL->addWidget(lbl);
			rowL->addWidget(m_heightSlider, 1);
			rowL->addWidget(m_heightVal);
			b->addWidget(rowW);
		}
		...
		// Speed row
		{
			...
			m_speedSlider->setRange(10, 500);
			m_speedSlider->setValue(m_dock->settings().speed);
			m_speedSlider->setStyleSheet(
				"QSlider::groove:horizontal{background:#334155;height:4px;border-radius:2px;}"
				"QSlider::handle:horizontal{background:#3b82f6;width:14px;height:14px;margin:-5px 0;border-radius:7px;}"
				"QSlider::sub-page:horizontal{background:#2563eb;border-radius:2px;}");
			m_speedVal = new QLabel(QString::number(m_dock->settings().speed));
			m_speedVal->setStyleSheet("color:#e2e8f0;font-size:11px;");
			m_speedVal->setMinimumWidth(36);
			m_speedVal->setAlignment(Qt::AlignRight | Qt::AlignVCenter);
			rowL->addWidget(lbl);
			rowL->addWidget(m_speedSlider, 1);
			rowL->addWidget(m_speedVal);
			b->addWidget(rowW);
		}
	...
	auto *typeSec = new CollapsibleSection("Typography");
	{
		...
		// Font size slider
		{
			...
			auto *rowL = new QHBoxLayout(rowW);
			rowL->setContentsMargins(0, 0, 0, 0);
			auto *lbl = new QLabel("Size");
			lbl->setStyleSheet(S_LABEL_MUTED);
			lbl->setSizePolicy(QSizePolicy::Preferred, QSizePolicy::Fixed);
			m_fontSizeSlider = new QSlider(Qt::Horizontal);
			m_fontSizeSlider->setRange(8, 200);
			m_fontSizeSlider->setValue(m_dock->settings().fontSize);
			m_fontSizeSlider->setStyleSheet(
				"QSlider::groove:horizontal{background:#334155;height:4px;border-radius:2px;}"
				"QSlider::handle:horizontal{background:#3b82f6;width:14px;height:14px;margin:-5px 0;border-radius:7px;}"
				"QSlider::sub-page:horizontal{background:#2563eb;border-radius:2px;}");
			m_fontSizeVal = new QLabel(QString::number(m_dock->settings().fontSize) + "px");
			m_fontSizeVal->setStyleSheet("color:#e2e8f0;font-size:11px;");
			m_fontSizeVal->setMinimumWidth(36);
			m_fontSizeVal->setAlignment(Qt::AlignRight | Qt::AlignVCenter);
			rowL->addWidget(lbl);
			rowL->addWidget(m_fontSizeSlider, 1);
			rowL->addWidget(m_fontSizeVal);
			b->addWidget(rowW);
		}
		...
		auto makeSimpleSliderRow = [&](const QString &label, int min, int max, int val, const QString &suffix,
		...
		connect(m_fontStyleCombo, QOverload<int>::of(&QComboBox::currentIndexChanged), this,
			&FormattingTab::onFontStyleChanged);
		connect(m_fontSizeSlider, &QSlider::valueChanged, this, &FormattingTab::onFontSizeChanged);
		connect(m_textColorBtn, &QPushButton::clicked, this, &FormattingTab::onTextColorPick);
		connect(m_separatorEdit, &QLineEdit::textChanged, this, &FormattingTab::onSeparatorChanged);
	}
	...
	auto *scaleSec = new CollapsibleSection("Text Scale");
	{
		...
		// Scale X  (50% – 300%, stored as 0.5 – 3.0)
		{
			...
			auto *rowL = new QHBoxLayout(rowW);
			rowL->setContentsMargins(0, 0, 0, 0);
			auto *lbl = new QLabel("Width Scale");
			lbl->setStyleSheet(S_LABEL_MUTED);
			lbl->setSizePolicy(QSizePolicy::Preferred, QSizePolicy::Fixed);
			...
			m_scaleXVal->setStyleSheet("color:#e2e8f0;font-size:11px;");
			m_scaleXVal->setMinimumWidth(36);
			m_scaleXVal->setAlignment(Qt::AlignRight | Qt::AlignVCenter);
			rowL->addWidget(lbl);
			rowL->addWidget(m_scaleXSlider, 1);
			rowL->addWidget(m_scaleXVal);
			b->addWidget(rowW);
		}
		...
		// Scale Y
		{
			...
			auto *rowL = new QHBoxLayout(rowW);
			rowL->setContentsMargins(0, 0, 0, 0);
			auto *lbl = new QLabel("Height Scale");
			lbl->setStyleSheet(S_LABEL_MUTED);
			lbl->setSizePolicy(QSizePolicy::Preferred, QSizePolicy::Fixed);
			...
			m_scaleYVal->setStyleSheet("color:#e2e8f0;font-size:11px;");
			m_scaleYVal->setMinimumWidth(36);
			m_scaleYVal->setAlignment(Qt::AlignRight | Qt::AlignVCenter);
			rowL->addWidget(lbl);
			rowL->addWidget(m_scaleYSlider, 1);
			rowL->addWidget(m_scaleYVal);
			b->addWidget(rowW);
		}
	...
	auto *clockSec = new CollapsibleSection("Clock", true); // open by default (clock is on)
	{
		...
		m_clockFormatCombo->setCurrentIndex(m_dock->settings().clockFormat == "12h" ? 1 : 0);
		cdLayout->addLayout(ctrlRow("Format", m_clockFormatCombo));

		// Clock font size
		// Helper lambda to create clock slider rows
		auto makeSliderRow = [&](QVBoxLayout *layout, const QString &lbl, QSlider *&slider, QLabel *&val,
					 int minV, int maxV, int curV, const QString &suffix) {
			auto *rowW = new QWidget;
			rowW->setStyleSheet("background:transparent;");
			auto *rowL = new QHBoxLayout(rowW);
			rowL->setContentsMargins(0, 0, 0, 0);
		...
		makeSliderRow(cdLayout, "Size", m_clockFontSizeSlider, m_clockFontSizeVal, 8, 150,
			      m_dock->settings().clockFontSize, "px");
		makeSliderRow(cdLayout, "W Scale", m_clockScaleXSlider, m_clockScaleXVal, 50, 300,
			      (int)(m_dock->settings().clockScaleX * 100.0), "%");
		makeSliderRow(cdLayout, "H Scale", m_clockScaleYSlider, m_clockScaleYVal, 50, 300,
			      (int)(m_dock->settings().clockScaleY * 100.0), "%");
		...
		connect(m_showClockCheck, &QCheckBox::toggled, this, &FormattingTab::onShowClockToggled);
		connect(m_clockFormatCombo, QOverload<int>::of(&QComboBox::currentIndexChanged), this,
			&FormattingTab::onClockFormatChanged);
		connect(m_clockFontSizeSlider, &QSlider::valueChanged, this, &FormattingTab::onClockFontSizeChanged);
		connect(m_clockScaleXSlider, &QSlider::valueChanged, this, &FormattingTab::onClockScaleXChanged);
		connect(m_clockScaleYSlider, &QSlider::valueChanged, this, &FormattingTab::onClockScaleYChanged);
		connect(m_clockSepColorBtn, &QPushButton::clicked, this, &FormattingTab::onClockSepColorPick);
	}
	...
	auto *presetSec = new CollapsibleSection("Presets");
	{
		...
		connect(m_presetNameEdit, &QLineEdit::returnPressed, this, &FormattingTab::onSavePreset);
...
void FormattingTab::refreshPresets()
{
	...
	for (const auto &preset : presets) {
		auto *row = new QWidget;
		row->setStyleSheet("background:#1e293b; border:1px solid #334155; border-radius:4px;");
		auto *rowLayout = new QHBoxLayout(row);
		rowLayout->setContentsMargins(8, 6, 8, 6);
		rowLayout->setSpacing(6);
		...
		loadBtn->setStyleSheet("QPushButton { background:#1e3a5f; color:#93c5fd; border-radius:4px;"
				       "  padding:3px 10px; border:none; font-size:11px; }"
				       "QPushButton:hover { background:#1d4ed8; }");
		loadBtn->setCursor(Qt::PointingHandCursor);
		QString capturedId = preset.id;
		connect(loadBtn, &QPushButton::clicked, this, [this, capturedId]() { m_dock->loadPreset(capturedId); });
